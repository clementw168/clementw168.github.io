...
/* --- Centered single image (max width) --- */
.img-center {
  display: flex;
...
.img-center img {
  max-width: 70%;   /* adjust to taste (50–80% works well) */
  height: auto;
  border-radius: 6px;
  display: block;
}
...
/* --- Mixed gallery (2x2 grid with varying heights) --- */
...
.img-gallery-mixed .tall {
  grid-row: span 2;
}

.img-gallery-mixed .wide {
  grid-column: span 2;
}
...
#### Collaboration with Assas Legal Innovation (05/03/2021)
